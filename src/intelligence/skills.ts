...
export class SkillScanner {
  ...
  scan(skillDirs: string[]): Skill[] {
    ...
    this.log.info(`Scanned ${skills.length} skills from ${skillDirs.length} directories`);
    return skills;
  }
}
...
export class SkillsManager {
   ...
   */
  findRelevant(query: string, topK: number = 3): Skill[] {
  ...
  buildPromptSection(maxChars: number = MAX_SKILLS_PROMPT_CHARS): string {
    ...
    lines.push("");
    lines.push("Use the load_skill tool to load a skill's full instructions when needed.");
