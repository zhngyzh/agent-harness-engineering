 ...
 */
function hashProjection(text: string, dim: number = 64): number[] {
...
export class MemoryStore {
   ...
   */
  search(query: string, topK: number = 5): MemorySearchResult[] {
    this.init();
    ...
    for (const fact of this.evergreenFacts) {
      const semanticScore = cosineSimilarity(hashProjection(fact.content), queryVec);
    ...
    this.log.debug(`Search "${query.slice(0, 30)}...": ${reranked.length} results`);
    return reranked;
  }
   ...
   */
  autoRecall(query: string, maxChars: number = 1000): string {
  ...
  getStats(): { evergreen: number; daily: number } {
    this.init();
    return { evergreen: this.evergreenFacts.length, daily: this.dailyFacts.length };
  }
  ...
  private loadDaily(): void {
    ...
    const files = readdirSync(this.dailyDir).filter((f) => f.endsWith(".jsonl"));
