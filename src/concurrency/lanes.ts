...
export class LaneQueue {
  ...
  enqueue<T>(execute: () => Promise<T>): Promise<T> {
    return new Promise<T>((resolve, reject) => {
      ...
      this.log.debug(`Task enqueued in lane "${this.name}": ${task.id} (queue: ${this.queue.length})`);
      this.processNext();
    });
  }
  ...
  reset(): void {
    ...
    this.queue = [];
    this.log.info(`Lane "${this.name}" reset (generation: ${this.currentGeneration})`);
  }
  ...
  private async processNext(): Promise<void> {
    ...
    if (task.generation < this.currentGeneration) {
      this.log.debug(`Dropping stale task ${task.id} (gen ${task.generation} < ${this.currentGeneration})`);
      task.reject(new Error("Task dropped: lane was reset"));
      this.processNext();
      return;
    }
    ...
    this.running++;
    this.log.debug(`Task started in lane "${this.name}": ${task.id} (active: ${this.running})`);
    ...
    try {
      ...
      this.running--;
      this.log.debug(`Task completed in lane "${this.name}": ${task.id} (active: ${this.running})`);
      this.processNext();
    }
  }
}
...
export class CommandQueue {
  ...
  getLane(name: string, maxConcurrency: number = 1): LaneQueue {
  ...
  getStats(): Array<{ name: string; queueLength: number; activeCount: number; generation: number }> {
