...
export class CronScheduler {
  ...
  private async runJob(job: CronJob): Promise<void> {
    ...
    try {
      ...
      this.log.error(`Cron job failed: ${job.name}`, { error: (err as Error).message });
      this.logJob(job, "error", (err as Error).message);

      if (job.consecutiveErrors >= this.maxConsecutiveErrors) {
        job.enabled = false;
        this.log.warn(`Job auto-disabled after ${this.maxConsecutiveErrors} errors: ${job.name}`);
      }
    }
  ...
  private logJob(job: CronJob, status: "success" | "error", error?: string): void {
    const entry = {
      jobId: job.id,
      jobName: job.name,
      status,
