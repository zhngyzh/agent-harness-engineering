...
import { PromptCache } from "../context/cache.js";
...
class ContextEngine {
  ...
  rebuild(): void {
    ...
    this._cacheRatio = PromptCache.cacheRatio(this._systemPrompt);
  }
  ...
  get cacheBlocks() {
    return PromptCache.split(this._systemPrompt);
  }
  ...
  async maybeCompact(messages: { length: number }[]): Promise<boolean> {
    if (this.compactor.shouldCompact(messages as any, 100_000)) {
...
async function main(): Promise<void> {
  ...
  const maxContextTokens = Number.parseInt(process.env.MAX_CONTEXT_TOKENS || "100000", 10);
  ...
  const ctxEngine = new ContextEngine(workspaceDir, llm, model, maxContextTokens);
  ctxEngine.init();
...
`);
  ...
  rl.on("line", async (line) => {
    ...
    if (input.startsWith("/")) {
      ...
      if (cmd === "/system") {
        ...
        if (args === "full") {
          ...
          console.log(`${DIM}${prompt.slice(0, 500)}... (${prompt.length} chars total)${RESET}`);
        }
        rl.prompt();
        return;
      }
      if (cmd === "/status") {
        ...
        console.log(`Memory: ${s.memory.evergreen} evergreen, ${s.memory.daily} daily`);
        ...
        if (args) {
          ...
          if (results.length === 0) {
            ...
            for (const r of results) {
              console.log(`  [${r.source}] (${r.score.toFixed(2)}) ${r.fact.content.slice(0, 80)}`);
            }
          }
        } else {
          const memStats = ctxEngine.memory.getStats();
          console.log(`${DIM}Evergreen: ${memStats.evergreen} facts | Daily: ${memStats.daily} facts`);
      ...
      if (cmd === "/skills") {
        ...
        if (skillList.length === 0) {
          console.log(`${DIM}No skills loaded. Place SKILL.md files in workspace/skills/.${RESET}`);
        } else {
          console.log(`${DIM}Loaded skills:${RESET}`);
          for (const s of skillList) {
            console.log(`  - ${s.frontmatter.name}: ${s.frontmatter.description.slice(0, 60)}`);
          }
        }
        rl.prompt();
        return;
      ...
      if (cmd === "/compact") {
        const msgs = [...loop.getMessages()];
        if (msgs.length <= 4) {
          console.log(`${YELLOW}Too few messages to compact (${msgs.length})${RESET}`);
        } else {
          const result = await ctxEngine.compactor.compact(msgs, "manual");
          if (result.compacted) {
            loop.reset();
            console.log(`${YELLOW}Compacted: ${result.record?.messagesBefore} → ${result.record?.messagesAfter} messages${RESET}`);
          } else {
      ...
      if (cmd === "/reload") {
        ...
        ctxEngine.rebuild();
        const s = ctxEngine.getStats();
        console.log(`${YELLOW}Context reloaded. Skills: ${s.skills}, Prompt: ${s.promptChars} chars${RESET}`);
        rl.prompt();
        return;
      }
    ...
    try {
      ...
      const response = await loop.sendMessage(enrichedInput, ctxEngine.systemPrompt);
