...
export class SkillGenerator {
   ...
   */
  analyzePatterns(toolSpans: ToolSpan[], minOccurrences: number = 3): SkillDraft[] {
    ...
    this.log.info(`Generated ${drafts.length} skill drafts from ${patterns.length} patterns`);
    return drafts;
  }
  ...
  listDrafts(): SkillDraft[] {
    ...
    for (const file of readdirSync(this.draftsDir)) {
      ...
      try {
        drafts.push(JSON.parse(readFileSync(join(this.draftsDir, file), "utf-8")));
      } catch {
        // skip
      }
    }
...
`;
  ...
  private generateSkillBody(pattern: ToolPattern): string {
    const steps = pattern.sequence.map((tool, i) => `${i + 1}. Use **${tool}**`).join("\n");
