...
const SECRET_PATTERNS = [
  ...
  { pattern: /-----BEGIN\s+(RSA|EC|DSA|OPENSSH)\s+PRIVATE\s+KEY-----/g, label: "Private key" },
...
const CORE_TOPICS = [
  "user", "preference", "project", "agent", "tool", "skill",
  "memory", "workflow", "config", "setting", "behavior",
  "rule", "constraint", "goal", "task", "context",
];
...
export class MemoryScanner {
  ...
  scan(content: string, existingEvergreen?: string): ScanResult {
    ...
    if (existingEvergreen) {
      const contradictions = this.detectContradictions(content, existingEvergreen);
    ...
    if (topicScore < 0.1 && content.length > 100) {
      findings.push({
        type: "off_topic",
        severity: "warning",
        description: "Content may be off-topic for agent memory (low topic relevance)",
      });
    }
   ...
   */
  private detectContradictions(content: string, evergreen: string): ScanFinding[] {
    const findings: ScanFinding[] = [];
    ...
    for (const cs of contentStatements) {
      for (const es of evergreenStatements) {
        ...
        if (cs.subject === es.subject && cs.predicate === es.predicate && cs.negated !== es.negated) {
          findings.push({
            type: "contradiction",
            severity: "warning",
  ...
  private extractStatements(text: string): Array<{ subject: string; predicate: string; negated: boolean; full: string }> {
    const statements: Array<{ subject: string; predicate: string; negated: boolean; full: string }> = [];
    // Simple pattern: "X is Y" / "X is not Y"
    const regex = /(\w[\w\s]{0,30})\s+(is|are|should|must|will)\s+(not\s+)?([^.!?\n]{3,50})/gi;
    let match;
  ...
  private logResult(result: ScanResult): void {
    ...
    if (!result.safe) {
      this.log.warn(`Memory scan: ${result.findings.length} findings for "${result.contentPreview}"`);
    } else {
