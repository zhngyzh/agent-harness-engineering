...
export interface Nudge {
  id: string;
  type: "tool_reminder" | "context_warning" | "error_pattern" | "memory_hint" | "review_based";
...
export class NudgeEngine {
  ...
  listAll(): Nudge[] {
    return [...this.nudges].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
  }
  ...
  private addNudges(newNudges: Nudge[]): void {
    ...
    for (const nudge of newNudges) {
      this.log.info(`Nudge created: [${nudge.priority}] ${nudge.message.slice(0, 60)}`);
