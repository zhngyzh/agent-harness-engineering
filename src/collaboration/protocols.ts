...
import { TeamMailbox, type TeamMessage } from "./team.js";

export type ShutdownState = "idle" | "requested" | "acknowledged" | "force" | "complete";
export type PlanState = "draft" | "pending" | "approved" | "rejected" | "timeout" | "executing" | "done";
...
export class ShutdownProtocol {
   ...
   */
  async initiate(from: string, to: string, reason?: string): Promise<ShutdownSession> {
    ...
    this.timers.set(
      requestId,
      setTimeout(() => this.handleTimeout(requestId), this.config.shutdownTimeoutMs),
    );
...
export class PlanApprovalProtocol {
  ...
  submit(submittedBy: string, plan: string, approver: string): PlanSession {
    ...
    this.timers.set(
      requestId,
      setTimeout(() => this.handleTimeout(requestId), this.config.planTimeoutMs),
    );
  ...
  reject(requestId: string, comment?: string): boolean {
    const session = this.sessions.get(requestId);
    if (!session || (session.state !== "pending" && session.state !== "draft")) return false;
    ...
    if (session.revisionCount >= session.maxRevisions) {
      ...
      this.log.info(`Plan returned for revision (${session.revisionCount}/${session.maxRevisions}): ${requestId}`);
    }
...
export class TaskHandoffProtocol {
  ...
  initiate(
    ...
    this.timers.set(
      requestId,
      setTimeout(() => this.handleTimeout(requestId), this.config.handoffTimeoutMs),
    );
