...
export type TaskStatus = "pending" | "claimed" | "in_progress" | "completed" | "failed" | "timeout";
...
export class TaskBoard {
  ...
  completeTask(taskId: string, result: string): boolean {
    const tasks = this.readAll();
    const task = tasks.find((t) => t.id === taskId);
    if (!task || !["claimed", "in_progress"].includes(task.status)) return false;
  ...
  getWorkload(agentName: string): AgentWorkload {
    ...
    return {
      agentName,
      activeTasks: agentTasks.filter((t) => ["claimed", "in_progress"].includes(t.status)).length,
  ...
  getStats(): Record<TaskStatus, number> {
    ...
    const stats: Record<TaskStatus, number> = {
      pending: 0, claimed: 0, in_progress: 0, completed: 0, failed: 0, timeout: 0,
    };
  ...
  private writeAll(tasks: Task[]): void {
    ...
    writeFileSync(tmpPath, tasks.map((t) => JSON.stringify(t)).join("\n") + "\n", "utf-8");
