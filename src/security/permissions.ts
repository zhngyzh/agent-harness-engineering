...
export class PermissionEngine {
  ...
  addRule(rule: PermissionRule): void {
    ...
    this.log.info(`Rule added: ${rule.pattern} → ${rule.decision} (priority ${rule.priority})`);
  }
  ...
  setAskHandler(handler: (request: PermissionRequest) => Promise<PermissionDecision>): void {
   ...
   */
  async check(subject: string, context?: Record<string, unknown>): Promise<PermissionResult> {
  ...
  checkSync(subject: string, context?: Record<string, unknown>): PermissionResult {
    const request: PermissionRequest = {
      subject,
      context,
      timestamp: new Date().toISOString(),
    ...
    const decision: PermissionDecision = matchedRule?.decision === "allow" ? "allow" : "deny";
