...
export class Sandbox {
	...
	checkFilePath(path: string, operation: "read" | "write"): SandboxCheck {
		const resolved = isAbsolute(path)
			? resolve(path)
			: resolve(this.resolvedWorkspace, path);
		...
		for (const blocked of [
			...
			if (
				resolved.startsWith(blockedResolved) ||
				resolved === blockedResolved
			) {
				return {
					allowed: false,
					reason: `Path blocked: ${path} (matches ${blocked})`,
				};
			}
		}
	...
	checkOutputSize(size: number): SandboxCheck {
		if (size > this.config.maxOutputSize) {
			return {
				allowed: false,
				reason: `Output size ${size} exceeds limit ${this.config.maxOutputSize}`,
			};
		}
		return { allowed: true };
	}
