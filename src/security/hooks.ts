...
export class HookSystem {
	...
	register(hook: Omit<Hook, "id">): Hook {
		...
		this.log.info(
			`Hook registered: ${fullHook.id} at ${hook.point} (priority ${hook.priority})`,
		);
		return fullHook;
	}
		...
		for (const hook of pointHooks) {
			...
			try {
				...
				if (result.action === "block") {
					...
					this.log.warn(
						`Hook blocked: ${hook.id} at ${point} — ${result.reason}`,
					);
					break;
				}
				...
				if (result.action === "modify") {
					...
					if (result.modifiedOutput)
						currentContext.output = result.modifiedOutput;
					if (result.modifiedMessage)
						currentContext.message = result.modifiedMessage;
					finalResult = result;
				}
			} catch (err) {
				const durationMs = Date.now() - startTime;
				...
				this.log.error(`Hook error: ${hook.id} at ${point}`, {
