...
const DELIMITER_PATTERNS = [
	...
	/\[SYSTEM\]/i,
	/\[INST\]/i,
	/###\s*(system|instructions?|human|assistant)/i,
];
...
const OBFUSCATION_PATTERNS = {
	...
	// biome-ignore lint/suspicious/noMisleadingCharacterClass: zero-width chars U+200B/U+200C/U+200D/U+FEFF
	zeroWidth: /[​‌‍﻿]/g,
...
export class InjectionDefense {
	...
	scan(input: string): ScanResult {
		...
		if (risk !== "none") {
			this.log.warn(
				`Injection scan: ${risk} risk, ${findings.length} findings`,
			);
			this.audit(result);
		}
	...
	private calculateRisk(findings: InjectionFinding[]): InjectionRisk {
		...
		const riskLevels: InjectionRisk[] = [
			"none",
			"low",
			"medium",
			"high",
