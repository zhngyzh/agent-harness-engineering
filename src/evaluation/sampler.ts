...
export class OnlineSampler {
	...
	constructor(workspaceDir: string, config: Partial<SamplerConfig> = {}) {
	...
	shouldSample(context: {
		...
		if (
			context.recentScore !== undefined &&
			context.recentScore < this.config.adaptiveThreshold
		) {
		...
		const totalSampled = [...this.taskTypeCounts.values()].reduce(
			(a, b) => a + b,
			0,
		);
		if (totalSampled > 0) {
	...
	recordSample(
		...
		this.log.debug(
			`Sample recorded: ${interaction.taskId} (${interaction.taskType}, reason: ${reason})`,
		);
		return sample;
	}
