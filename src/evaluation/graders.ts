	...
	grade(output: string, evalCase: EvalCase): GradeResult {
		...
		if (evalCase.expected !== undefined) {
			...
			if (normalizedOutput === normalizedExpected) {
				...
				details.push(
					`Exact match: FAIL (expected ${normalizedExpected.length} chars, got ${normalizedOutput.length})`,
				);
				failures++;
				passed = false;
			}
		...
		if (evalCase.expectedSchema) {
			...
			try {
				...
				if (missing.length === 0) {
					...
					details.push(
						`JSON schema: FAIL (missing fields: ${missing.join(", ")})`,
					);
					failures++;
					passed = false;
				}
	...
	grade(output: string, evalCase: EvalCase): GradeResult {
		...
		if (rubric.length === 0) {
			return {
				...
				score: 1,
				level: 2,
				feedback: "No rubric items to check",
			};
		}
		...
		for (const item of rubric) {
			...
			if (item.startsWith("keyword:")) {
				...
				const min = Number.parseInt(item.slice(11), 10);
				...
				const max = Number.parseInt(item.slice(11), 10);
...
export class EvalRunner {
	...
	private save(result: EvalResult): void {
		...
		this.log.info(
			`Eval: case ${result.caseId} — score ${result.finalScore}, passed ${result.passed}`,
		);
	}
}
