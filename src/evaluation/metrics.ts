...
export class MetricsCalculator {
	...
	computePassAtK(
		...
		const successes = [...taskTrials.values()].filter((trials) =>
			trials.some((t) => t.passed),
		).length;
	 ...
	 */
	wilsonLowerBound(successes: number, total: number, z = 1.96): number {
		if (total === 0) return 0;
