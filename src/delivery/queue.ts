...
export class DeliveryQueue {
	...
	markFailed(id: string, error: string): void {
		...
		if (msg.attempts >= DELIVERY_MAX_RETRIES) {
			...
			this.log.warn(
				`Message failed permanently: ${id} (${msg.attempts} attempts)`,
			);
		} else {
			msg.status = "pending";
			...
			this.writeAtomic(id, msg);
			this.log.info(
				`Message retry scheduled: ${id} (attempt ${msg.attempts}, delay ${delay}s)`,
			);
		}
	}
	...
	getStats(): { pending: number; failed: number; total: number } {
		...
		if (existsSync(this.failedDir)) {
			const failedFiles = readdirSync(this.failedDir).filter((f) =>
				f.endsWith(".json"),
			);
