...
import { MAX_TOTAL_RETRIES, PER_PROFILE_RETRY } from "../core/constants.js";
	...
	rotate(reason: FailoverReason): AuthProfile | null {
		...
		while (attempts < this.profiles.length) {
			...
			if (
				...
				new Date(profile.cooldownUntil) > new Date()
			) {
				attempts++;
				continue;
			}
			...
			this.log.info(
				`Profile rotated: ${this.profiles[startIndex].label} -> ${profile.label} (${reason})`,
			);
			return profile;
		}
...
export class ResilienceRunner {
	...
	async execute(
		...
		while (totalAttempts < MAX_TOTAL_RETRIES) {
			...
			try {
				this.log.debug(
					`LLM call attempt ${totalAttempts} (profile: ${profile.label})`,
				);
				...
				this.log.warn(`LLM call failed: ${reason}`, {
				...
				if (reason === FailoverReason.CONTEXT_OVERFLOW) {
					throw new ResilienceError(
						"Context overflow - compaction needed",
						reason,
					);
				}
