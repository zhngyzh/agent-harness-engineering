...
import {
	SESSION_FILE_EXT,
	SESSION_META_FILE_PREFIX,
	SESSION_META_FILE_SUFFIX,
} from "./constants.js";
...
export class SessionStore {
	...
	constructor(workspaceDir: string, sessionId?: string) {
		...
		this.filePath = join(
			this.sessionDir,
			`${this.sessionId}${SESSION_FILE_EXT}`,
		);
	...
	static listSessions(workspaceDir: string): SessionMeta[] {
		...
		for (const f of allFiles) {
			if (f.endsWith(SESSION_FILE_EXT)) {
				...
				const id = f.slice(
					SESSION_META_FILE_PREFIX.length,
					-SESSION_META_FILE_SUFFIX.length,
				);
		...
		return Array.from(sessionIds).map((id) => {
			...
			const metaPath = join(
				sessionDir,
				`${SESSION_META_FILE_PREFIX}${id}${SESSION_META_FILE_SUFFIX}`,
			);
	...
	private metaFilePath(): string {
		return join(
			this.sessionDir,
			`${SESSION_META_FILE_PREFIX}${this.sessionId}${SESSION_META_FILE_SUFFIX}`,
		);
	}
