...
export const bashTool: Tool = {
  ...
  handler: async (input) => {
    ...
    try {
      ...
      throw new ToolError(command, msg, "Check the command syntax and file paths. Use 'ls' to verify file exists.");
    }
  },
};
...
export const readFileTool: Tool = {
  ...
  handler: async (input) => {
    ...
    if (!fs.existsSync(resolved)) {
      throw new ToolError(filePath, `File not found: ${filePath}`, "Use list_directory to find the correct path.");
    }
...
export class ToolRegistry {
  ...
  async execute(name: string, input: Record<string, unknown>): Promise<string> {
    ...
    if (!tool) {
      throw new Error(
        `Unknown tool: "${name}". Available tools: ${this.list().map((t) => t.name).join(", ")}`,
      );
    }
