		...
		return {
			...
			content: msg.content.map((block) => {
				if (
					...
					block.content.length > MICRO_COMPACT_TRUNCATE_CHARS
				) {
					return {
						...block,
						content: `${block.content.slice(0, MICRO_COMPACT_TRUNCATE_CHARS)}\n... (truncated ${block.content.length} → ${MICRO_COMPACT_TRUNCATE_CHARS} chars)`,
					};
				}
				return block;
			}),
...
export class ContextCompactor {
	...
	private async summarize(messages: Message[]): Promise<string> {
		const summaryPrompt = `Summarize the following conversation concisely. Preserve: key facts, decisions made, tool call outcomes, file paths, errors encountered. Be brief — aim for 200-400 words.\n\n${messages.map((m) => this.messageToText(m)).join("\n---\n")}`;
		...
		try {
			...
			this.log.error("Summarization failed, using fallback", {
