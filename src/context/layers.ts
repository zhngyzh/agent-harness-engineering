...
import type { Message, AgentConfig } from "../core/types.js";
...
export class ContextLayerManager {
   ...
   */
  assemble(config: AgentConfig, residentFiles: Map<string, string>, skillSummary: string, memories: string): string {
    const parts: string[] = [];
   ...
   */
  estimateUsage(messages: Message[], maxTokens: number): { estimatedTokens: number; ratio: number } {
    const text = messages
      .map((m) => {
