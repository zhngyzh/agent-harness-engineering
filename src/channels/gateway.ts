...
export class Gateway implements Channel {
  ...
  resolveAgent(msg: InboundMessage): string | null {
    for (const binding of this.bindings) {
      if (binding.tier === 1 && binding.peerId === msg.peerId) return binding.agentName;
      if (binding.tier === 2 && binding.guildId === msg.guildId) return binding.agentName;
      if (binding.tier === 3 && binding.accountId === msg.accountId) return binding.agentName;
      if (binding.tier === 4 && binding.channel === msg.channel) return binding.agentName;
    ...
    this.wss.on("connection", (ws) => {
      ...
      ws.on("message", (data) => {
        try {
          ...
          this.log.error("Failed to parse message", { error: (err as Error).message });
        }
      });
