...
import { MAX_FILE_CHARS, MAX_TOTAL_BOOTSTRAP_CHARS } from "../../src/core/constants.js";
...
describe("P2: Bootstrap file size limits", () => {
  ...
  it("enforces total bootstrap size limit (150KB)", () => {
    ...
    const files = ["SOUL.md", "IDENTITY.md", "TOOLS.md", "MEMORY.md", "USER.md", "HEARTBEAT.md", "BOOTSTRAP.md", "AGENTS.md"];
    ...
    expect(result.totalChars).toBeLessThanOrEqual(MAX_TOTAL_BOOTSTRAP_CHARS + 20);
...
describe("P2: SelfReview file creation", () => {
  ...
  it("multiple analyze() calls append to the same daily file", () => {
    ...
    const spans: ToolSpan[] = [
      { tool: "bash", input: {}, startedAt: "T1", endedAt: "T2", durationMs: 100 },
      { tool: "read_file", input: {}, startedAt: "T3", error: "not found", endedAt: "T4", durationMs: 50 },
    ];
  ...
  it("TF-IDF IDF is now correctly recalculated for all tokens", () => {
    ...
    class TFIDF {
      ...
      search(query: string): Array<{ index: number; score: number }> {
        const queryTokens = query.toLowerCase().replace(/[^\w\s]/g, " ").split(/\s+/).filter(t => t.length > 1);
