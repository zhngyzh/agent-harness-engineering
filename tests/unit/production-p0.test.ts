...
describe("P0: AgentLoop tool error recovery", () => {
	...
	it("feeds tool errors back to model and allows recovery", async () => {
			...
			if (callCount === 2) {
				...
				return createResponse(
					[
						{
							type: "text",
							text: "File not found, but I can try listing the directory instead.",
						},
					],
					"end_turn",
				);
		...
		const content = Array.isArray(toolResultMsg.content)
...
describe("P0: InjectionDefense /g flag intermittent failure", () => {
	...
	it("isSafe returns consistent results on repeated calls (no /g flag corruption)", () => {
		...
		const results = Array.from({ length: 10 }, () =>
			defense.isSafe(injectionInput),
		);
		expect(results.every((r) => r === false)).toBe(true);

		// Clean input — should always be safe
		const cleanResults = Array.from({ length: 10 }, () =>
			defense.isSafe("Hello, how are you?"),
		);
		...
		llm.messages = async () =>
			createResponse(
				[
					{
						type: "text",
						text: "This is a partial response that was cut off...",
					},
				],
				"max_tokens",
			);
