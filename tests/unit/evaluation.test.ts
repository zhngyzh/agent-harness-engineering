...
import {
	...
	OnlineSampler,
} from "../../src/evaluation/sampler.js";
...
describe("DeterministicGrader", () => {
	...
	beforeEach(() => {
		grader = new DeterministicGrader();
	});
...
describe("LLMJudgeGrader", () => {
				...
				({
					messages: async (): Promise<LLMResponse> => ({
						content: [
							{
								type: "text",
								text: '{"score": 0.8, "passed": true, "feedback": "Good quality output"}',
							},
						],
						stopReason: "end_turn",
	...
	it("handles LLM error gracefully", async () => {
		...
		const result = await errorGrader.grade("output", {
			id: "1",
			input: "test",
		});
		expect(result.passed).toBe(false);
	...
	it("handles unparseable LLM response", async () => {
		...
		const result = await weirdGrader.grade("output", {
			id: "1",
			input: "test",
		});
		expect(result.passed).toBe(false);
