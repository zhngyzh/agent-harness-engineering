...
import { LaneQueue, CommandQueue } from "../../src/concurrency/lanes.js";
...
describe("LaneQueue", () => {
  it("executes tasks in FIFO order", async () => {
    const lane = new LaneQueue("test", 1, { debug: () => {}, info: () => {}, warn: () => {}, error: () => {} } as any);
  ...
  it("tracks queue length and active count", async () => {
    ...
    const p1 = lane.enqueue(() => new Promise((r) => setTimeout(() => r(1), 50)));
  ...
  it("supports concurrency > 1", async () => {
    const lane = new LaneQueue("test", 3, { debug: () => {}, info: () => {}, warn: () => {}, error: () => {} } as any);
    let concurrent = 0;
    let maxConcurrent = 0;
  ...
  it("rejects task on error", async () => {
    ...
    await expect(lane.enqueue(async () => {
  ...
  it("drops stale tasks after reset", async () => {
    ...
    const p1 = lane.enqueue(() => new Promise<number>((r) => { resolveP1 = r; }));
    const p2 = lane.enqueue(async () => 2);
    const p3 = lane.enqueue(async () => 3);
...
describe("CommandQueue", () => {
  ...
  it("isolates lanes — tasks in different lanes don't block each other", async () => {
    ...
    const pa = cq.enqueue("lane-a", () => new Promise<string>((r) => setTimeout(() => { results.push("a"); r("a"); }, 50)));
    const pb = cq.enqueue("lane-b", async () => { results.push("b"); return "b"; });
  ...
  it("resetAll clears queued tasks in all lanes", async () => {
    ...
    const pa1 = cq.enqueue("lane-a", () => new Promise<number>((r) => { resolvePA = r; }));
    const pa2 = cq.enqueue("lane-a", async () => 2);
    const pb1 = cq.enqueue("lane-b", () => new Promise<number>((r) => { resolvePB = r; }));
  ...
  it("getStats returns lane statistics", async () => {
    ...
    const pa = cq.enqueue("stats-a", () => new Promise<number>((r) => setTimeout(() => r(1), 50)));
