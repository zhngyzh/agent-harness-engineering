...
import { McpToolBridge, type McpServerConfig, type McpTool, type McpToolCallResult } from "../../src/core/mcp-bridge.js";
import { ToolRegistry, createDefaultRegistry } from "../../src/core/tool-registry.js";
...
function mockMcpTool(name: string, description: string, properties: Record<string, unknown> = {}): McpTool {
  return {
    name,
    description,
    inputSchema: {
...
describe("McpToolBridge", () => {
  ...
  describe("tool name wrapping", () => {
    it("wraps tool names with server prefix", () => {
      ...
      const mcpTool = mockMcpTool("search", "Search the web", { query: { type: "string" } });
...
describe("McpToolBridge integration", () => {
  ...
  describe("registry integration", () => {
    ...
    it("handles MCP tool execution errors gracefully", async () => {
      ...
      await expect(registry.execute("test:failing_tool", { input: "test" })).rejects.toThrow(
