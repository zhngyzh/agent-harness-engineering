...
describe("ShutdownProtocol", () => {
  ...
  beforeEach(() => {
    ...
    protocol = new ShutdownProtocol(mailbox, DEFAULT_PROTOCOL_CONFIG, shutdownHandler);
  });
  ...
  it("lists all sessions", () => {
    protocol.handleRequest({
      id: "msg-1", type: "shutdown_request", from: "a", to: "b",
      content: "test", request_id: "s1", timestamp: new Date().toISOString(), read: false,
    });
    protocol.handleRequest({
      id: "msg-2", type: "shutdown_request", from: "c", to: "d",
      content: "test", request_id: "s2", timestamp: new Date().toISOString(), read: false,
    });
...
describe("PlanApprovalProtocol", () => {
  ...
  it("submits a plan for approval", () => {
    const session = protocol.submit("agent-1", "Step 1: Do X\nStep 2: Do Y", "reviewer");
  ...
  it("calls onPlanApproved callback", () => {
    ...
    const p = new PlanApprovalProtocol(mailbox, DEFAULT_PROTOCOL_CONFIG, onApproved, onRejected);
...
describe("TaskHandoffProtocol", () => {
  ...
  it("acknowledges a handoff", () => {
    const session = protocol.initiate("alice", "bob", "Task", "ctx", "0%", ["step 1"]);
...
describe("TaskBoard", () => {
  ...
  it("claims a task", () => {
    const task = board.addTask({
      title: "Task 1", description: "desc", priority: "medium",
      timeoutMs: 60_000, tags: [],
    });
  ...
  it("claims highest-priority pending task", () => {
    board.addTask({ title: "Low", description: "d", priority: "low", timeoutMs: 60_000, tags: [] });
    board.addTask({ title: "Critical", description: "d", priority: "critical", timeoutMs: 60_000, tags: [] });
    board.addTask({ title: "High", description: "d", priority: "high", timeoutMs: 60_000, tags: [] });
  ...
  it("updates progress", () => {
    const task = board.addTask({
      title: "Task", description: "d", priority: "medium",
      timeoutMs: 60_000, tags: [],
    });
    board.claimTask(task.id, "agent-1");
  ...
  it("gets agent workload", () => {
    const t1 = board.addTask({ title: "T1", description: "d", priority: "high", timeoutMs: 60_000, tags: [] });
    const t2 = board.addTask({ title: "T2", description: "d", priority: "low", timeoutMs: 60_000, tags: [] });
  ...
  it("lists tasks filtered by status", () => {
    const t1 = board.addTask({ title: "T1", description: "d", priority: "high", timeoutMs: 60_000, tags: [] });
    board.addTask({ title: "T2", description: "d", priority: "low", timeoutMs: 60_000, tags: [] });
    board.claimTask(t1.id, "agent-1");
