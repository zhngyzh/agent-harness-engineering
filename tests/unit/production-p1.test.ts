...
import { ContextCompactor, microCompact } from "../../src/context/compaction.js";
...
import type { LLMResponse, LLMClient, Message } from "../../src/core/types.js";
...
describe("P1: Context compaction", () => {
  ...
  it("microCompact truncates long tool results", () => {
    ...
    const messages: Message[] = [
      ...
      { role: "user", content: [{ type: "tool_result", tool_use_id: "t1", content: longContent }] },
    ];

    const compacted = microCompact(messages);
    const resultBlock = (compacted[2].content as Array<{ type: string; content: string }>)[0];
  ...
  it("compact reduces message count", async () => {
    ...
    llm.messages = async () =>
      createResponse(
        [{ type: "text", text: "Summary: discussed various topics including files and commands." }],
        "end_turn",
      );
    ...
    const summaryText = (result.messages[2].content as Array<{ type: string; text: string }>)[0].text;
...
describe("P1: Subagent isolation", () => {
  ...
  it("subagent receives task message but not parent history", async () => {
    ...
    const manager = new SubagentManager(
      () => ({
        messages: async (_m, system, msgs, _t, _max) => {
          ...
          return createResponse([{ type: "text", text: "subagent done" }], "end_turn");
        },
      } as LLMClient),
    ...
    expect((capturedMessages[0].content as Array<{ text: string }>)[0].text).toBe("do something");
  });
});
...
describe("P1: Lane FIFO ordering", () => {
  it("processes tasks in FIFO order within a lane", async () => {
    ...
    const promises = [
      ...
      lane.enqueue(async () => { order.push(4); }),
      lane.enqueue(async () => { order.push(5); }),
    ];
...
describe("P1: Tracing tool span lifecycle", () => {
  ...
  it("save writes trace to disk", () => {
    ...
    tracing.addToolSpan({
      type: "tool_start", sessionId: "s1", timestamp: "T1",
...
describe("P1: Full session lifecycle", () => {
  ...
  it("multi-turn session with tools, tracing, and persistence", async () => {
    ...
    llm.messages = async () => {
      ...
      switch (callCount) {
        case 1:
          return createResponse([{ type: "text", text: "你好！我是 Luna。" }], "end_turn");
        case 2:
        ...
        case 3:
          return createResponse([{ type: "text", text: "目录中有 test.txt。" }], "end_turn");
        case 4:
          return createResponse(
        ...
        default:
          return createResponse([{ type: "text", text: "test.txt 包含 Hello World。" }], "end_turn");
      }
    };
...
describe("P1: Subagent abort", () => {
  ...
  it("abort stops a running subagent", async () => {
    const manager = new SubagentManager(
      () => ({
        messages: () => new Promise((r) => setTimeout(() => r(
          createResponse([{ type: "text", text: "done" }], "end_turn")
        ), 5000)),
      } as LLMClient),
      "You are a test subagent.",
    );
...
describe("P1: InjectionDefense false positives and sanitization", () => {
  ...
  it("normal code-related inputs are not flagged as critical injection", () => {
    ...
    for (const input of normalInputs) {
      ...
      expect(result.risk !== "high" && result.risk !== "critical").toBe(true,
  ...
  it("detects zero-width characters", () => {
    ...
    expect(result.findings.some((f) =>
      f.description.includes("Zero-width")
    )).toBe(true);
  });
});