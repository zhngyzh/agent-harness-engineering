...
import { ProfileManager, ResilienceRunner, FailoverReason, ResilienceError } from "../../src/delivery/resilience.js";
...
describe("Gateway - 5-Tier Routing", () => {
  it("resolves tier 1 (peer_id) binding", () => {
    ...
    const msg: InboundMessage = {
      id: "1", channel: "ws", accountId: "a1", peerId: "user-123",
      text: "hi", timestamp: "", metadata: {},
    };
  ...
  it("resolves tier 2 (guild_id) binding", () => {
    ...
    const msg: InboundMessage = {
      id: "1", channel: "ws", accountId: "a1", peerId: "user-1",
      guildId: "guild-456", text: "hi", timestamp: "", metadata: {},
    };
  ...
  it("falls back to default when no binding matches", () => {
    ...
    const msg: InboundMessage = {
      id: "1", channel: "ws", accountId: "a1", peerId: "unknown",
      text: "hi", timestamp: "", metadata: {},
    };
  ...
  it("returns null when no bindings exist", () => {
    const gw = new Gateway("127.0.0.1", 0);
    const msg: InboundMessage = {
      id: "1", channel: "ws", accountId: "a1", peerId: "user",
      text: "hi", timestamp: "", metadata: {},
    };
  ...
  it("builds session key with per-account-channel-peer scope", () => {
    ...
    const msg: InboundMessage = {
      id: "1", channel: "telegram", accountId: "bot-1", peerId: "user-123",
      text: "hi", timestamp: "", metadata: {},
    };
...
describe("Resilience - ProfileManager", () => {
  const profiles: AuthProfile[] = [
    ...
    { apiKey: "key-2", label: "secondary", cooldownUntil: null, failureCount: 0 },
    { apiKey: "key-3", label: "tertiary", cooldownUntil: null, failureCount: 0 },
  ];
  ...
  it("skips profiles in cooldown", () => {
    const pm = new ProfileManager([
      { apiKey: "key-1", label: "p1", cooldownUntil: new Date(Date.now() + 60000).toISOString(), failureCount: 0 },
      { apiKey: "key-2", label: "p2", cooldownUntil: new Date(Date.now() + 60000).toISOString(), failureCount: 0 },
...
describe("Resilience - ResilienceRunner", () => {
  ...
  it("rotates profile on auth error", async () => {
    ...
    const runner = new ResilienceRunner(pm, () => {
      ...
      if (callCount === 1) {
        ...
        client.messages = async () => { throw new Error("401 Unauthorized"); };
        return client;
      }
  ...
  it("throws ResilienceError when context overflow", async () => {
    ...
    const runner = new ResilienceRunner(pm, () => {
      ...
      client.messages = async () => { throw new Error("context length exceeded max_tokens"); };
      return client;
    });
