...
import { cacheRatio, splitCacheBlocks } from "../../src/context/cache.js";
import {
	ContextCompactor,
	microCompact,
...
describe("SystemPromptBuilder", () => {
	it("builds a prompt with all layers", () => {
		...
		const prompt = builder.build({
			...
			skills: [{ name: "code-review", description: "Review code for quality" }],
			memories: [{ content: "User prefers TypeScript.", score: 0.9 }],
			runtime: {
...
describe("Micro-Compact", () => {
	it("truncates long tool results", () => {
		...
		const resultBlock = (
			compacted[2].content as { type: string; content: string }[]
		)[0];
	...
	it("preserves short tool results", () => {
		...
		const compacted = microCompact(messages);
		const resultBlock = (
			compacted[0].content as { type: string; content: string }[]
		)[0];
...
Dynamic content here.`;
		...
		const blocks = splitCacheBlocks(prompt);
